    ...
    for row1 in query1:
        ...
        with open(filename, "w") as fw:
            query2 = session.execute(
                """select mchtid, sum(amount), count(1) from fs_dz_result_tmp where settledate = :settledate and trans_chnl = :trans_chnl and fscode = :fscode and bankcode = :bankcode group by mchtid order by mchtid """,
                {
                    'settledate': settle_date, 'trans_chnl': trans_chnl,
            ...
            query3 = session.execute(
                """select mchtid, mchtname, cardno, transtime, termssn, termid, refnbr, amount from fs_dz_result_tmp where settledate = :settledate and trans_chnl = :trans_chnl and fscode = :fscode and bankcode = :bankcode order by mchtid """,
            ...
            fw.write("\n".join(
                ["%15.15s|%-40.40s|%-19.19s|%10.10s|%6.6s|%8.8s|%12.12s|%12.12s" % tuple(row3) for row3 in query3]))
...
def process_on_report(session, settle_date, batch_no, ss_upload):
    ...
    if batch_no:
        query1 = session.execute(
            """select fscode, bankcode from fs_dz_result_his where trans_chnl in (:trans_chnl0, :trans_chnl1, :trans_chnl2) and settledate = :settledate and mchtid in (select mchtid from fs_dz_zdsh_lb where batch_no = :batch_no) group by fscode, bankcode order by fscode, bankcode""",
            {
                'settledate': settle_date, 'trans_chnl0': trans_chnl0, 'trans_chnl1': trans_chnl1, 'trans_chnl2': trans_chnl2, 'batch_no': batch_no
            })
    else:
        query1 = session.execute(
            """select fscode, bankcode from fs_dz_result_his where trans_chnl in (:trans_chnl0, :trans_chnl1, :trans_chnl2) and settledate = :settledate group by fscode, bankcode order by fscode, bankcode""",
            {
                'settledate': settle_date, 'trans_chnl0': trans_chnl0, 'trans_chnl1': trans_chnl1, 'trans_chnl2': trans_chnl2
            })
    ...
    for row1 in query1:
        ...
        with open(filename, "w") as fw:
            query2 = session.execute(
                """select sum(amount), count(1) from fs_dz_result_his where settledate = :settledate and trans_chnl in (:trans_chnl0, :trans_chnl1, :trans_chnl2) and fscode = :fscode and bankcode = :bankcode""",
                {
                    'settledate': settle_date, 'trans_chnl0': trans_chnl0, 'trans_chnl1': trans_chnl1, 'trans_chnl2': trans_chnl2,
                    'fscode': row1[0], 'bankcode': row1[1]
                })
            ...
            query3 = session.execute(
                """select mchtid, mchtname, refnbr, cardno, transtime, amount, trans_type, fs_ins_id from fs_dz_result_his where settledate = :settledate and trans_chnl in (:trans_chnl0, :trans_chnl1, :trans_chnl2) and fscode = :fscode and bankcode = :bankcode order by mchtid """,
                {
                    'settledate': settle_date, 'trans_chnl0': trans_chnl0, 'trans_chnl1': trans_chnl1, 'trans_chnl2': trans_chnl2,
                    'fscode': row1[0],
                    'bankcode': row1[1]
...
def main():
    ...
    try:
        ...
        ss_upload = SsUpload(settle_date, ProcessMode.ON)
